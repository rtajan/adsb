...
struct _enumerate_impl_inner
{
    using type = typename _enumerate_impl<index + 1, TS...>::type::template prepend<type_list_item<T, index>>;
};
...
struct type_list
{
    ...
    static auto _for_each_and_collect_helper(const Func& func) -> U<decltype(func(TS()))...>
    {
...
/*template <typename type_list> struct type_list_length
{
enum
{
    value = type_list::length
};
};


template <typename type_list, unsigned int index = 0, bool stop = ( index ==
type_list_length<type_list>::value )> struct type_switch
{
void operator()( unsigned int i)
{
    if( i == index )
    return f.template operator()<typename type_listGet<type_list,
index>::type>(); else
    {
    TypeSwitch<type_list, index + 1> next;
